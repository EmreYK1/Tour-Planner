...
export class RouteMapComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  private updateMap(): void {
    ...
    if (this.geometry) {
      ...
      L.marker([first[1], first[0]]).addTo(this.map!).bindPopup('Start');
      L.marker([last[1], last[0]]).addTo(this.map!).bindPopup('Ende');

  } else {
    this.map!.setView([48.2082, 16.3738], 12);
    L.marker([48.2082, 16.3738]).addTo(this.map!).bindPopup('Wien').openPopup();
  }
}
